...
class ZscoreModel(Model):
    ...
    def __init__(self, name, parameters, tables, credentials, storage_credentials, other, internal_source, model_table, model_schema, stream, delay, train_type):
        super().__init__("zscore_model", parameters, tables, credentials, storage_credentials,
    ...
    def train(self, stream):
        ...
        df['timestamp'] = pd.to_datetime(df['ingestion_at'])
        df['timestamp'] = pd.to_datetime(df.timestamp).dt.tz_convert('UTC')
        df = df.sort_values('timestamp')
        df = df[~df.cod_stock_source.str.contains('PST')]
        df_grouped = df.groupby(['name_store', 'timestamp']).sum()
        ...
        df_grouped[str(roll_avg)+'day_rolling_avg_presale'] = df_grouped.presale_units.rolling(roll_avg).mean()
        df_grouped[str(roll_avg)+'day_rolling_std'] = df_grouped.qty.rolling(roll_avg).std()
        df_grouped[str(roll_avg)+'dayEWM'] = df_grouped.qty.ewm(roll_avg).mean()

        #calculate now timestamp
        timeZ_At = pytz.timezone('UTC')
        dt_At = datetime.now(timeZ_At)
        hora = dt_At.strftime("%Y-%m-%d %H:00:00")
        ...
        df_grouped_indexed = df_grouped_indexed.loc[:,
                             ['name_store', 'timestamp', str(roll_avg)+'day_rolling_avg', str(roll_avg)+'day_rolling_std', str(roll_avg)+'dayEWM', str(roll_avg)+'day_rolling_avg_presale']]
        df_grouped_indexed.columns = ['NAME_STORE', 'TIMESTAMP', 'DAY_ROLLING_AVG', 'DAY_ROLLING_STD', 'DAY_EWM','PRESALES_AVG']
    ...
    def detect(self, stream, alert):
        ...
        dataframe['timestamp'] = pd.to_datetime(dataframe['ingestion_at'], utc=False)
        ...
        dataframe_grouped = dataframe.groupby(['name_store', 'timestamp']).sum()
        ...
        tabla = pd.merge(df_grouped_indexed, df_coef, on=["name_store"])
        tabla = tabla.dropna()
        ...
        tabla['z_score_presales'] = (tabla['presale_units'] - tabla['presales_avg']) / abs(tabla['mean_gap'])
        ...
        data = dataframe_grouped_indexed[dataframe_grouped_indexed['timestamp'] == actual_time].sort_values(by=['qty'], ascending=False)
        data_ant = dataframe_grouped_indexed[dataframe_grouped_indexed['timestamp'] == last_time].sort_values(by=['qty'], ascending=False)
        data_ant = data_ant.loc[:, ['name_store', 'qty']]
        data = pd.merge(data, data_ant, on=["name_store"], how='outer')
        data['change']=data['qty_x']-data['qty_y']
        data = data.loc[:, ['name_store', 'change']]
        tabla = pd.merge(tabla, data, on=["name_store"], how='outer')
        ...
        for i in range(0, len(tabla)):
            ...
            elif (abs(tabla['z_score'][i]) > 4.5 and abs(tabla['mean_gap'][i]) > 150000 and tabla['z_score_presales'][i] < 0.31):
                tabla['z_score_indicador'][i] = 1
            elif (abs(tabla['z_score'][i]) > 4.5 and tabla['z_score_presales'][i] > 0.30):
                tabla['z_score_indicador'][i] = 2
            elif (abs(tabla['change'][i]) > 50000):
                tabla['z_score_indicador'][i] = 3
                tabla['mean_gap'][i] = tabla['change'][i]
            elif (abs(tabla['z_score'][i]) > 4.5 and abs(tabla['mean_gap'][i]) < 150001 and tabla['z_score_presales'][i] < 0.31):
                tabla['z_score_indicador'][i] = 3
            else:
                tabla['z_score_indicador'][i] = 0

        # tabla: select some columns and filter some rows
        tabla_filtered = tabla[tabla['z_score_indicador'] > 0]
        ...
        for i in range(0, (len(tabla))):
            ...
            actual_time = dataframe[dataframe['name_store'] == tabla['name_store'][i]]['timestamp'].unique()[-1]
            last_time = dataframe[dataframe['name_store'] == tabla['name_store'][i]]['timestamp'].unique()[-2]

            data = dataframe[dataframe['name_store'] == tabla['name_store'][i]][dataframe['timestamp'] == actual_time].sort_values(by=['qty'], ascending=False)
            data_ant = dataframe[dataframe['name_store'] == tabla['name_store'][i]][dataframe['timestamp'] == last_time].sort_values(by=['qty'], ascending=False)
            data = pd.merge(data, data_ant, on=["cod_stock_source"], how='outer')
            data['qty_x'] = data['qty_x'].fillna(0)
            data['qty_y'] = data['qty_y'].fillna(0)
            data['change'] = abs(data['qty_x'] - data['qty_y'])
            data['%change'] = ((data['qty_x'] - data['qty_y']) / data['qty_y']) * 100
